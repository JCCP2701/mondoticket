...
- **`create-checkout.ts`** (POST, autenticado): recibe `{orderId}`, lee la orden y sus `order_items` **del lado servidor** (nunca confía en un monto que mande el navegador), arma el request de Checkout a OrkestaPay (línea de "Cargo por servicio" calculada como `total - subtotal` para que la suma cuadre exacto centavo a centavo; `expires_at` fijo en `300000` ms — 5 min, la duración de la sesión de checkout, confirmado con soporte de OrkestaPay, independiente de `organizations.reservation_hold_minutes`), reclama la orden con un valor sentinel antes de llamar a OrkestaPay (protege contra dos invocaciones concurrentes del mismo endpoint, no solo contra la carrera con el webhook), lo crea, guarda `orkesta_checkout_id`/`orkesta_order_id` en la orden, regresa `{checkoutRedirectUrl}`. Ver `orkestapay-pending-fixes.md` para el detalle del bug de concurrencia que este mecanismo corrige.
- **`webhook.ts`** (POST, sin auth de sesión — la confianza viene de la firma Svix): lee el **body crudo** con `req.on('data'/'end')` directamente (nunca `req.body`, que ya viene re-serializado y rompería la firma), verifica con `new Webhook(secret).verify(rawBody, headers)`, dedup por `svix-id` en `payment_webhook_events`, busca la orden por `orkesta_order_id`, y despacha: `payment.purchase`/`payment.capture` → `confirm_order_paid`; `payment.cancel` → `do_release_order`; `payment.refund` → `cancel_order_tickets_for_gateway_refund` (reembolsos hechos fuera de la app, p.ej. directo en el dashboard de OrkestaPay). Si no encuentra la orden (carrera con `create-checkout` aún escribiendo), responde `404` para que Svix reintente solo — nunca `200` en ese caso.
...
> Ver también `orkestapay-pending-fixes.md`: hallazgos de una auditoría posterior (race condition en `create-checkout.ts`, saneamiento de errores, manejo de `payment.refund` en el webhook, y el fix de `expires_at` del checkout) — **ya implementados en código**; solo falta aplicar la migración `0046_gateway_refund_webhook.sql` contra Supabase y desplegar a producción.
