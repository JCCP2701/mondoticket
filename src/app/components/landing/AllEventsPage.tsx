...
const inputStyle: React.CSSProperties = {
    padding: '12px 16px', borderRadius: '12px', border: '1px solid rgba(139,92,246,0.25)',
    background: 'rgba(255,255,255,0.03)', color: '#f0edff', fontSize: '14px', outline: 'none',
};
        ...
        <div style={{ minHeight: '100vh', background: '#0d0b1e' }}>
            ...
            <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '140px 24px 60px' }}>
                <div style={{ marginBottom: '40px' }}>
                    <h1 style={{ fontSize: '40px', fontWeight: 900, color: '#f0edff', marginBottom: '12px', letterSpacing: '-1px' }}>
                        Todos los <span className="tb-gradient-text">Eventos</span>
                    </h1>
                    <p style={{ fontSize: '16px', color: 'rgba(240,237,255,0.5)' }}>
                ...
                <div style={{
                    ...
                    padding: '20px', borderRadius: '20px', background: 'rgba(19,16,42,0.8)', border: '1px solid rgba(139,92,246,0.15)',
                }}>
                    <div style={{ position: 'relative', flex: '1 1 220px' }}>
                        <Search size={16} color="rgba(240,237,255,0.4)" style={{ position: 'absolute', left: '14px', top: '50%', transform: 'translateY(-50%)' }} />
                        <input
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <label style={{ fontSize: '12px', color: 'rgba(240,237,255,0.4)' }}>Desde</label>
                        <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} style={inputStyle} />
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <label style={{ fontSize: '12px', color: 'rgba(240,237,255,0.4)' }}>Hasta</label>
                        <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} style={inputStyle} />
                    </div>

                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', color: 'rgba(240,237,255,0.6)', cursor: 'pointer' }}>
                ...
                {loading && <p style={{ textAlign: 'center', color: 'rgba(240,237,255,0.4)' }}>Cargando eventos...</p>}

                {!loading && filtered.length === 0 && (
                    <div style={{ textAlign: 'center', padding: '80px 20px', color: 'rgba(240,237,255,0.4)' }}>
                        ...
                        {filtered.map((event, i) => {
                            ...
                            return (
                                <Link
                                    ...
                                    style={{
                                        borderRadius: '20px', overflow: 'hidden', background: 'rgba(19,16,42,0.8)',
                                        border: '1px solid rgba(139,92,246,0.15)', textDecoration: 'none', display: 'block',
                                    }}
                                >
                                    ...
                                    <div style={{ padding: '18px' }}>
                                        <h3 style={{ fontSize: '17px', fontWeight: 800, color: '#f0edff', marginBottom: '10px' }}>{event.name}</h3>
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '14px' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'rgba(240,237,255,0.5)', fontSize: '13px' }}>
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '14px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                                            <p style={{ fontSize: '18px', fontWeight: 900, color: '#f0edff' }}>{priceFrom(event)}</p>
            ...
            <style>{`.tb-card-hover:hover .event-image { transform: scale(1.08); transition: transform 0.5s ease; }`}</style>
        </div>
    );
}
