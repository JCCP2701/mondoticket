-- Attribution column for staff-sold orders (taquilla and, as of this
-- migration, promotor): who actually executed the sale, as opposed to
-- orders.user_id which is the buyer (already always null for walk-in/
-- taquilla-style sales). Populated by create_order_and_tickets below
-- whenever the caller isn't buying for themselves (the same branch that
-- already authorizes "creating an order for another user"); left null for
-- plain online self-checkout, where there is no seller distinct from the
-- buyer. No ON DELETE clause, matching orders.user_id's existing unadorned
-- `references profiles(id)` — staff accounts are never hard-deleted today
-- (invite-staff.ts only calls auth.admin.deleteUser as a same-request
-- rollback, never as a standalone "remove this account" feature).
alter table orders add column sold_by uuid references profiles(id);

create index idx_orders_sold_by on orders(sold_by) where sold_by is not null;

-- Same 11-arg signature as 0036 (no new parameter — attribution is derived
-- from auth.uid(), already computed as v_uid), so CREATE OR REPLACE is
-- safe and keeps the existing `grant execute ... to authenticated` intact
-- (0036 had to drop+recreate only because it renamed a parameter, which
-- CREATE OR REPLACE genuinely can't do — this migration doesn't).
...
-- Analytics RPC for a promoter's own dashboard: goal (ticket count) vs
-- accumulated sales per configured period, plus commission earned if the
-- organization configured one (promoter_terms). Recomputed live from
-- orders/tickets, same "no ledger" model as get_broker_transactions — a
-- refund (which only ever sets tickets.status='cancelled', see
-- refund_tickets in 0013/0019) is automatically excluded from both the
-- ticket count and the revenue/commission on the next call.
create or replace function get_promoter_progress()
returns table (
...
begin
  ...
  with sold as (
    ...
    join event_ticket_types ett on ett.id = t.ticket_type_id
    where o.status = 'paid' and o.sold_by = auth.uid()
  )
  select
    pg.organization_id,
    org.name,
    ...
    pg.period_start,
    pg.period_end,
    pg.target_ticket_count,
    coalesce(s.cnt, 0)::bigint,
    coalesce(s.rev, 0)::numeric,
    pt.commission_percentage,
    case when pt.commission_percentage is not null
      then round(coalesce(s.rev, 0) * pt.commission_percentage / 100, 2)
      else null
    end
  from promoter_goals pg
  join organizations org on org.id = pg.organization_id
  left join promoter_terms pt
    on pt.promoter_profile_id = pg.promoter_profile_id and pt.organization_id = pg.organization_id
  left join lateral (
    select count(*) as cnt, sum(price) as rev
    from sold
    where sold.organization_id = pg.organization_id
      and sold.paid_at >= pg.period_start
      and sold.paid_at < pg.period_end + interval '1 day'
  ) s on true
  where pg.promoter_profile_id = auth.uid()
  order by pg.period_start desc;
end;
$$;

grant execute on function get_promoter_progress() to authenticated;
