-- Corrección de producto sobre 0026: un boleto tiene UN solo QR, el mismo
-- sin importar si se ve en la app, se manda por correo, se imprime, o se
-- exporta a Apple/Google Wallet — no un código que rota cada 30s solo
-- dentro de la app. La protección anti-clonación sigue siendo real: en
-- cuanto ese código se escanea una vez, check_in_ticket lo "quema"
-- (valid -> used, atómico) y cualquier copia posterior (screenshot,
-- reenvío) deja de servir — solo entra la primera persona que lo presente.
--
-- allow_static_qr pasa de "solo boletos vendidos en taquilla" a "todos los
-- boletos, sin importar el canal de venta".
...
update tickets set allow_static_qr = true where allow_static_qr = false;
